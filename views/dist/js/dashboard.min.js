...
angular.module('RDash', ['ngMaterial','ngAnimate','ui.bootstrap', 'ui.router', 'ui.tree','ngRoute', 'ngCookies', 'gantt', 'gantt.table', 'gantt.movable', 'gantt.tooltips','gantt.tree','chart.js','ntt.TreeDnD']);
"use strict";
...
angular.module('RDash').service('Tarea', function($rootScope, Http, EventHandler) {
    ...
    self.getEstimaciones = function(tareaId, anio, semana){
        return Http.get('tareas/'+tareaId+'/estimaciones/'+anio+'/'+semana);
    };

    self.getNotas = function(tareaId){
        return Http.get('notas/tarea/'+tareaId);
    };
...
angular.module('RDash').service('Nota', function($rootScope, Http, EventHandler) {
    console.log('SERVICIO_NOTA');
    self = this;
    ...
    self.get = function(notaId) {
        return Http.get('notaId/' + notaId);
    };

    self.save = function(nota) {
        return Http.post('notas/', nota);
    };

    self.delete = function(regitroId) {
        return Http.delete('notas/'+ regitroId);
    };
...
angular.module('RDash').service('Utilidad', function($rootScope, Http, EventHandler) {
    ...
    self.getSemanas = function() {
        return Http.get('utilidades/semanas');
    };

});

angular.module('RDash').service('Reporte', function($rootScope, Http, EventHandler) {
    console.log('SERVICIO_Reporte');
    self = this;


    self.getAll = function(dptoId, anio, semIni, semFin) {
        return Http.get('reportes/rptAll/' + dptoId+'_'+anio+'_'+semIni+'_'+semFin);
    };
...
angular.module("RDash").config(["$stateProvider", "$urlRouterProvider", function(t, e) {
    e.otherwise("/"), t.state("index", {
    ...
    }).state("reporteExcelAll", {
        url: "/reportes/reporteExcelAll",
        templateUrl: "../app/tareas/reporteAll.html"
    }).state("registro", {
        url: "/tareas/registro",
        ...
        url: "/tareas/nota",
        templateUrl: "../app/tareas/addNota.html"
    }).state("departamentos", {
        url: "/departamentos",
...
angular.module("RDash").controller('LoginCtrl', function($rootScope, $scope, $location, $route/*, $mdDialog*/, Colaborador, Utilidad, EventHandler) {
...
angular.module("RDash").controller('reportesCtrl', function($scope,  $location, Reporte, Utilidad) {
    console.log('CONTROLADOR_DPTO');
    $scope.selected = [];
    ...
    function refresh() {
        if(!(idUsuarioActual)){
            ...
            $scope.urlRpt = null;
            $scope.urlRptCrr = null;
            $scope.urlRptPln = null;
            Utilidad.getSemanas().then(function(semanas) {
                var semTmp = [];
                var anio;
                for(var i = 0; i<semanas.length; i++){
                    var semVar = {
                        id: semanas[i].semana,
                        label: semanas[i].semana+' - ('+semanas[i].fecini+' a '+semanas[i].fecfin+')'
                    };
                    semTmp.push(semVar);
                    anio = semanas[i].anio;
                }
                semTmp.sort(function(a,b) {return (a.id > b.id) ? 1 : ((b.id > a.id) ? -1 : 0);} );
                $scope.semanas = semTmp;
                $scope.filtro = {
                  anio: anio
                }
            });
    ...
    $scope.generarReporte = function() {
        var deptoId = idUsuarioActual.DepartamentoId;
        if(idUsuarioActual.rol == 'D'){
            deptoId = 0;
        }
        $scope.urlRpt = 'api/reportes/rptAll/'+deptoId+'_'+$scope.filtro.anio+'_'+$scope.filtro.semIni+'_'+$scope.filtro.semFin;
        $scope.urlRptCrr = 'api/reportes/rptCerr/'+$scope.filtro.anio+'_'+$scope.filtro.semIni+'_'+$scope.filtro.semFin;
        $scope.urlRptPln = 'api/reportes/rptPlnd/'+$scope.filtro.anio+'_'+$scope.filtro.semIni+'_'+$scope.filtro.semFin;
        //Reporte.getAll(idUsuarioActual.DepartamentoId, $scope.filtro.semIni,$scope.filtro.semFin);
    };
...
angular.module("RDash").controller('notaCtrl', function($scope, $location, Nota) {
    console.log('CONTROLADOR_NOT');
    ...
    $scope.guardarNota = function() {
        var notaTmp = {
            id: $scope.nota.id,
            fecha: $scope.nota.fecha,
            texto: $scope.nota.texto,
            TareaId: $scope.nota.TareaId
        };
        $scope.promise = Nota.save(notaTmp).then(function(){
            $location.path('/reportes/listaTareas');
        }).$promise;
    };
...
angular.module("RDash").controller('estimacionCtrl', function($scope, $location, Estimacion, Utilidad, Tarea) {
    console.log('CONTROLADOR_REG');
    ...
    $scope.guardarEstimacion = function() {
        $scope.promise = Tarea.getEstimaciones($scope.estimacion.TareaId, $scope.estimacion.anio ,$scope.estimacion.semana).then(function(tareas){
            var idEstm = $scope.estimacion.id;
            if(tareas.length >0 ){
                idEstm = tareas[0].estimaciones[0].id;
            }
            var estimacionTmp = {
                id: idEstm,
...
angular.module("RDash").controller('listaTareasController', function($scope, $location, $mdDialog, Proyecto, Subproyecto, Tarea, Colaborador, Utilidad) {
    console.log('CONTROLADOR_gantt');
    $scope.selected = [];
    ...
    function refresh() {
        ...
        $scope.my_tree.addNota = function (node) {
            Tarea.get(node.Id).then(function(tarea){
                idTareaActual = tarea;
            }).then(function(){
                $location.path('/tareas/nota');
            });
        ...
        $scope.my_tree.verDescripcion = function(node) {
            $mdDialog.show(
              $mdDialog.alert()
                .clickOutsideToClose(true)
                .title(node.Nombre)
                .textContent(node.descripcion)
        ...
        $scope.my_tree.verNota = function(node) {
            Tarea.getNotas(node.Id).then(function(tareas){
                var notas = [];
                for  (var i= 0 ; i < tareas.length; i++){
                    notas.push('-->'+tareas[i].texto);
                }
                $mdDialog.show(
                  $mdDialog.alert()
                    .clickOutsideToClose(true)
        ...
        $scope.col_defs = [
            ...
            {
                displayName:  'Ver Descripcion',
                cellTemplate: '<button ng-show="node.descripcion "  ng-click="tree.verDescripcion(node)" class="btn btn-default btn-sm">VerDescripcion</button>'
            },
            {
                displayName:  'Ver Notas',
                cellTemplate: '<button ng-show="node.descripcion "  ng-click="tree.verNota(node)" class="btn btn-default btn-sm">VerNotas</button>'
            },
            {
                displayName:  'Adicionar Nota',
                cellTemplate: '<button ng-show="node.descripcion "  ng-click="tree.addNota(node)" class="btn btn-default btn-sm">Adc.Nota</button>'
            },
            {
                field: 'estadoId',
                displayName:  'Estado',
                    ...
                    for (var j = 0; j < proyectos[i].subproyectos.length; j++) {
                        ...
                        for ( var k = 0; k < proyectos[i].subproyectos[j].tareas.length; k++) {
                            ...
                            for ( var l = 0; l < proyectos[i].subproyectos[j].tareas[k].registros.length; l++) {
                                ...
                                if(proyectos[i].subproyectos[j].tareas[k].registros[l].anio == anioActual && proyectos[i].subproyectos[j].tareas[k].registros[l].semana >= (semanaActual) && proyectos[i].subproyectos[j].tareas[k].registros[l].semana <= (semanaActual+1)){
                                        adicionarTarea = true;
                                }
                            }
                            for ( l = 0; l < proyectos[i].subproyectos[j].tareas[k].estimaciones.length; l++) {
                                ...
                                if(proyectos[i].subproyectos[j].tareas[k].estimaciones[l].anio == anioActual &&  proyectos[i].subproyectos[j].tareas[k].estimaciones[l].semana >= (semanaActual) && proyectos[i].subproyectos[j].tareas[k].estimaciones[l].semana <= (semanaActual+1)){
                                        adicionarTarea = true;
                                        sinEstimacion = false;
                                        console.log('JLPO:'+sinEstimacion);
                                }
                            }
                            ...
                            var dataTrPry = {
                                ...
                                descripcion: proyectos[i].subproyectos[j].tareas[k].descripcion,
                                estadoId: proyectos[i].subproyectos[j].tareas[k].estado,
                                __icon_class__: "fa fa-table"
                            };
                            console.log('['+proyectos[i].subproyectos[j].tareas[k].id+']['+proyectos[i].subproyectos[j].tareas[k].semana+']['+semanaActual+']adicionarTarea:['+adicionarTarea+']['+sinEstimacion+']');
                            if (proyectos[i].subproyectos[j].tareas[k].estado == 'C' && ( proyectos[i].subproyectos[j].tareas[k].semana < semanaActual || proyectos[i].subproyectos[j].tareas[k].anio != anioActual)){
                                adicionarTarea = false;
                                sinEstimacion = false;
                            }
...
angular.module("RDash").controller('listaTareasSemController', function($scope, $location, Proyecto, Subproyecto, Tarea, Colaborador, Utilidad) {
    ...
    function refresh() {
        ...
        $scope.refreshTareas = function(){
            ...
            var anioNum = $scope.filtro.anioNum;
            var deptoId = 0;
            var semTmp = [];
            ...
            Utilidad.getSemanas().then(function(semanas) {
                for(var i = 0; i<semanas.length; i++){
                    var semVar = {
                        anio: semanas[i].anio,
                        id: semanas[i].semana,
                        label: semanas[i].fecini+'|'+semanas[i].fecfin
                    };
                    if(semanas[i].anio == anioActual){
                      semTmp.push(semVar);
                    }
                }
                return semTmp;
                ...
                for (var i = 0; i < colaboradores.length; i++) {
                    ...
                    for (var j = 0; j < colaboradores[i].tareas.length; j++){
                        ...
                        for ( var k = 0; k < colaboradores[i].tareas[j].registros.length; k++) {
                            ...
                            anioTarea = colaboradores[i].tareas[j].registros[k].anio;
                            if(semanas.indexOf(semTarea) < 0 && anioTarea == anioActual){
                                var labelSemana = semTarea;
                                for(var indTarea = 0; indTarea< semTmp.length; indTarea++){
                                    if(semTmp[indTarea].id == semTarea){
                                        labelSemana = semTmp[indTarea].label;
                                    }
                                }
                                dataSemana = {
                                    anio: anioTarea,
                                    semana: semTarea,
                                    labelSem: labelSemana,
                                    horasRep: 0,
                                    horasPlan: 0,
                                    __children__: []
                                }
                                if(anioTarea = anioActual){
                                  console.log('Anio No: '+anioActual+' anioTarea : '+anioTarea);
                                  if(semanaNum > 0){
                                      ...
                                      console.log('Semana No: '+(semanaActual - 2)+' semTarea : '+semTarea);
                                      if((semanaActual - 2) <= semTarea){
                                          semanas.push(semTarea);
                                          dataSemanas.push(dataSemana);
                                      }
                                  }
                        ...
                        for ( k = 0; k < colaboradores[i].tareas[j].estimaciones.length; k++) {
                            ...
                            anioTarea = colaboradores[i].tareas[j].estimaciones[k].anio;
                            dataSemana = {};
                            if(semanas.indexOf(semTarea) < 0 && anioTarea == anioActual){
                                var labelSemana = semTarea;
                                for(var indTarea = 0; indTarea< semTmp.length; indTarea++){
                    ...
                    for(var indSem = 0; indSem < dataSemanas.length; indSem++){
                        for (j = 0; j < colaboradores[i].tareas.length; j++){
                            ...
                            for ( k = 0; k < colaboradores[i].tareas[j].registros.length; k++) {
                                if(dataSemanas[indSem].anio == colaboradores[i].tareas[j].registros[k].anio &&
                                  dataSemanas[indSem].semana == colaboradores[i].tareas[j].registros[k].semana){
                                    horasRep = parseFloat(horasRep) + parseFloat(colaboradores[i].tareas[j].registros[k].horas);
                                }
                            }
                            for ( k = 0; k < colaboradores[i].tareas[j].estimaciones.length; k++) {
                                if(dataSemanas[indSem].anio == colaboradores[i].tareas[j].estimaciones[k].anio &&
                                  dataSemanas[indSem].semana == colaboradores[i].tareas[j].estimaciones[k].semana){
