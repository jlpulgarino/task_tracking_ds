...
angular.module('RDash').service('Proyecto', function($rootScope, Http, EventHandler) {
    ...
    self.getGantt = function(colabrid, proyectoId, subproyectoId, estadoTarea){
        var filtroGantt=''+colabrid+'_'+subproyectoId+'_'+estadoTarea;
...
angular.module("RDash").controller('editTareasCtrl', function($scope, /*$mdDialog, $mdEditDialog,*/ $location, Proyecto, Subproyecto, Departamento, Tarea, Colaborador) {
    ...
    function refreshSb() {
        ...
        $scope.deptoId = idSubProyActual.DepartamentoId;
        Colaborador.getByDepto(idSubProyActual.DepartamentoId).then(function(colaboradores) {
...
angular.module("RDash").controller('ganttController', function($scope, $location, Proyecto, Subproyecto, Colaborador) {
    ...
    $scope.refreshGantt = function(){
        ...
        console.log('Colaborador::'+clbrId);
        Proyecto.getGantt(clbrId,pryId,0,estTarea).then(function(proyectos){
...
angular.module("RDash").controller('listaTareasController', function($scope, $location, Proyecto, Subproyecto, Tarea, Colaborador, Utilidad) {
    ...
    function refresh() {
        ...
        $scope.my_tree.asignar = function (node) {
            Tarea.get(node.Id).then(function(tarea){
                idTareaActual = tarea;
                return tarea;
            }).then(function(tareaEmulada){
                tareaEmulada.ColaboradorId = node.idColb;
        ...
        $scope.my_tree.estimar = function (node) {
            ...
            var startDay = 0;
            value = value - (86400000 * ((7 + today.getDay() - startDay) % 7));
            weekStart = new Date(value);
            value = value + (86400000 * ((6 - today.getDay() - startDay) % 7));
            weekEnd = new Date(value);
            Utilidad.getSemana(today).then(function(semanaCalc){
                semana = semanaCalc.semana;
                console.log('semana::'+semana);
            });

            console.log('today.valueOf():'+today.valueOf()+' , weekStart:'+weekStart+' :: weekEnd: '+weekEnd);
            */
        };
        ...
        $scope.col_defs = [
            ...
            },
            {
                ...
                cellClass:   'v-middle',
                displayName: 'Id',
                cellTemplate: '<select ng-show="!node.nombreColb "  name="idColb" ng-model="node.idColb" ng-options="colaborador.id as colaborador.nombre for colaborador in colaboradores" ></select>'
            },
            {
                displayName:  'Asignar',
                cellTemplate: '<button ng-show="!node.nombreColb " ng-click="tree.asignar(node)" class="btn btn-default btn-sm">Asignar</button>'
            }
        ];
            ...
            //var estTarea = $scope.filtro.estadoTarea;
            //var pryId = $scope.filtro.pryId;
            /*Proyecto.getGantt(0,pryId,0,estTarea).then(function(proyectos){*/
            var colbrId = 999999;
            if(!(idUsuarioActual)){
                $location.path('/login');
            }else{
            ...
            Proyecto.getGantt(colbrId,0,0,'T').then(function(proyectos){
                for (var i = 0; i < proyectos.length; i++) {
                    ...
                    for (var j = 0; j < proyectos[i].subproyectos.length; j++) {
                        ...
                        for ( var k = 0; k < proyectos[i].subproyectos[j].tareas.length; k++) {
                            ...
                            console.log('numDias='+proyectos[i].subproyectos[j].tareas[k].dias);
